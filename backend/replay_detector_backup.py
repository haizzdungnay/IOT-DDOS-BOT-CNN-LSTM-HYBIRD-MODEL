...
class ReplayDetector:
    ...
    def __init__(self, models_dir='backend/models', data_dir='data'):
        ...
        # Stats with Confusion Matrix for each model
        self.stats = {
            'total_packets': 0,
            'true_attacks': 0,
            'true_normal': 0,
            'consensus_attacks': 0,
            # CNN metrics
            'cnn_tp': 0, 'cnn_tn': 0, 'cnn_fp': 0, 'cnn_fn': 0,
            # LSTM metrics
            'lstm_tp': 0, 'lstm_tn': 0, 'lstm_fp': 0, 'lstm_fn': 0,
            # Hybrid metrics
            'hybrid_tp': 0, 'hybrid_tn': 0, 'hybrid_fp': 0, 'hybrid_fn': 0
        }
    ...
    def _replay_loop(self, features, labels, metadata, speed, callback):
        ...
        for i in range(len(features)):
            ...
            if len(self.buffer) == self.TIME_STEPS:
                ...
                # Update confusion matrix for each model
                for model_name, pred_data in predictions.items():
                    pred = pred_data['pred']
                    prefix = model_name.lower()
                    
                    if true_label == 1 and pred == 1:
                        self.stats[f'{prefix}_tp'] += 1  # True Positive
                    elif true_label == 0 and pred == 0:
                        self.stats[f'{prefix}_tn'] += 1  # True Negative
                    elif true_label == 0 and pred == 1:
                        self.stats[f'{prefix}_fp'] += 1  # False Positive
                    elif true_label == 1 and pred == 0:
                        self.stats[f'{prefix}_fn'] += 1  # False Negative
    ...
    def get_stats(self):
        ...
        for prefix in ['cnn', 'lstm', 'hybrid']:
            tp = stats[f'{prefix}_tp']
            tn = stats[f'{prefix}_tn']
            ...
            # Accuracy
            stats[f'{prefix}_accuracy'] = ((tp + tn) / total * 100) if total > 0 else 0
            
            # Precision
            stats[f'{prefix}_precision'] = (tp / (tp + fp) * 100) if (tp + fp) > 0 else 0
            
            # Recall (TPR)
            stats[f'{prefix}_recall'] = (tp / (tp + fn) * 100) if (tp + fn) > 0 else 0
            
            # FPR
            stats[f'{prefix}_fpr'] = (fp / (fp + tn) * 100) if (fp + tn) > 0 else 0
